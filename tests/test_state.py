...
class TestCompactionAndPruning(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory()
        self.path = Path(self.tmp.name) / "state.json"
    ...
    def _aged(self, state, cve_id, days, **kw):
        from datetime import datetime, timedelta, timezone
        v = Vuln(cve_id=cve_id, platform="ios", severity="HIGH", **kw)
        rec = state.record_cve(v)
        rec.last_updated = (datetime.now(timezone.utc) - timedelta(days=days)).isoformat(timespec="seconds")
        return rec

    def test_one_line_per_cve(self):
        s = State(self.path)
        for i in range(50):
            s.record_cve(Vuln(cve_id=f"CVE-2026-{1000+i}", platform="ios", severity="HIGH"))
        s.save()
        text = self.path.read_text()
        self.assertEqual(sum(1 for ln in text.splitlines() if '"CVE-2026-' in ln), 50)
        self.assertTrue(len(text.splitlines()) < 80, "50 CVEs should not exceed ~80 lines")

    def test_roundtrip_after_compaction(self):
        s = State(self.path)
        s.record_cve(Vuln(cve_id="CVE-2026-1001", platform="ios", severity="CRITICAL", kev=True))
        s.upsert_release(release(), "new")
        s.save()
        s2 = State(self.path)
        self.assertTrue(s2.cves["CVE-2026-1001"].kev)
        self.assertIn("ios-26-6-1", s2.releases)

    def test_prune_drops_stale_unreferenced(self):
        s = State(self.path)
        self._aged(s, "CVE-2026-1001", days=400)
        self.assertEqual(s.prune()["cves_pruned"], 1)

    def test_prune_never_drops_alerted(self):
        """Dropping an alerted CVE resets the dedupe flag and re-pages on-call."""
        s = State(self.path)
        self._aged(s, "CVE-2026-1001", days=400)
        s.mark_alerted("CVE-2026-1001")
        self.assertEqual(s.prune()["cves_pruned"], 0)

    def test_prune_never_drops_kev_or_exploited(self):
        s = State(self.path)
        self._aged(s, "CVE-2026-1001", days=400, kev=True)
        self._aged(s, "CVE-2026-1002", days=400, exploited=True)
        self.assertEqual(s.prune()["cves_pruned"], 0)

    def test_prune_keeps_cves_referenced_by_live_release(self):
        s = State(self.path)
        s.upsert_release(release(cves=("CVE-2026-1001",)), "new")
        self._aged(s, "CVE-2026-1001", days=999)
        self.assertEqual(s.prune()["cves_pruned"], 0)

    def test_prune_keeps_undelivered_releases_forever(self):
        """Pruning a release makes it look new again and files a duplicate ticket."""
        from datetime import datetime, timedelta, timezone
        s = State(self.path)
        rec = s.upsert_release(release(), "new")
        rec.last_seen = (datetime.now(timezone.utc) - timedelta(days=900)).isoformat(timespec="seconds")
        self.assertEqual(s.prune()["releases_pruned"], 0, "undelivered release must survive")
