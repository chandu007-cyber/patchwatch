...
class ReleaseRecord:
    ...
    # Set ONLY after a successful SMTP send. This is what stops a release being
    # emailed twice; a failed send leaves it null so the next run retries.
...
class State:
    ...
    def upsert_release(self, release: Any, status: str) -> ReleaseRecord:
        ...
        if key in self.releases:
            ...
            if status == "changed":
                ...
                rec.email_sent_at = None  # and re-include it in the next digest
            rec.last_seen = now
        else:
            rec = ReleaseRecord(
                release_key=key,
    ...
    def pending_email(self) -> list[str]:
        """Release keys never successfully emailed."""
        return [k for k, r in self.releases.items() if not r.email_sent_at]

    def mark_emailed(self, release_keys: list[str]) -> None:
        stamp = utcnow()
        for key in release_keys:
            if key in self.releases:
                self.releases[key].email_sent_at = stamp
        self.meta["last_email_at"] = stamp

    @property
    def last_email_at(self) -> str | None:
        return self.meta.get("last_email_at")
