...
def render_update(entries: list[dict], out_path: str | Path, *, run_number: str = "") -> Path:
    """One document covering ONLY this update's new releases.

    Separate from the cumulative timeline on purpose: this is the thing you read
    when the update lands, so it must contain nothing you have already seen.
    """
    now = datetime.now(timezone.utc)
    exploited = sum(e.get("exploited_count", 0) for e in entries)
    worst = min((e["worst"] for e in entries), key=ORDER.index, default="UNKNOWN")

    body = [
        ox.para("Patch Update", style="Title"),
        ox.para([ox.run(now.strftime("%d %B %Y  %H:%M UTC")
    ...
    if exploited:
        body.append(ox.para([
            ox.run(f"{exploited} CVE{'s' if exploited != 1 else ''} in this update "
                   f"{'are' if exploited != 1 else 'is'} under active exploitation "
                   f"or listed in CISA KEV. Patch these first.",
                   bold=True, color="8B0000"),
        ]))

    rows = [[ox.run("Metric", bold=True), ox.run("Value", bold=True)]]
    rows.append([ox.run("New releases in this update"), ox.run(str(len(entries)))])
    rows.append([ox.run("Highest severity"),
                 ox.run(worst, bold=True, color=SEVERITY_COLOR.get(worst, "525252"))])
    rows.append([ox.run("CVEs across these releases"),
                 ox.run(str(sum(e.get("cve_count", 0) for e in entries)))])
    rows.append([ox.run("Exploited / KEV"),
                 ox.run(str(exploited), bold=True,
                        color="8B0000" if exploited else "525252")])
    body.append(ox.table(rows, widths=[5000, 5050]))

    body.append(ox.para("New releases", style="Heading1"))
    if not entries:
        body.append(ox.para("No new releases in this update."))
    for entry in sorted(entries, key=lambda e: (ORDER.index(e["worst"]), e["title"])):
