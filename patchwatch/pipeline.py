...
from .deliver import mailer as mailer_mod
...
class RunSummary:
    ...
    def render(self) -> str:
        lines = [
            ...
            f"  emailed releases     {self.emailed_releases}"
            + (f"  ({self.email_status})" if self.email_status else ""),
...
def run(
    *,
    ...
    # ---- timeline report ----------------------------------------------------
    # Append to history.json, then regenerate the Word document from it. Runs
    # unconditionally: no credentials, nothing to expire, nothing to approve.
    if not dry_run and work:
        entries = [
            timeline_mod.entry_from_release(r, status, analyses.get(r.release_key, {}))
            for r, status, _ in work
        ]
        ...
        # Regenerate even when nothing was added, so a deleted .docx self-heals.
        out = timeline_mod.render(history, docx_path)
        summary.timeline_path = str(out)
        print(f"  timeline: +{added} entr{'y' if added == 1 else 'ies'} "
              f"({len(history)} total) -> {out}")
    elif dry_run:
        print(f"  timeline: [dry-run] would add {len(work)} entr"
              f"{'y' if len(work) == 1 else 'ies'}")

    # ---- email digest (optional) ---------------------------------------------
    # ONE digest per run, covering only releases never successfully emailed, subject
    # to a 4h floor read from state rather than inferred from the cron schedule -
    # so manual runs and re-runs cannot bypass it.
    if not dry_run:
        skip, why = mailer_mod.throttled(state.last_email_at)
        pending_keys = set(state.pending_email())
        to_email = [r for r, _, _ in work if r.release_key in pending_keys]

        if skip:
            summary.email_status = f"throttled: {why}"
            print(f"  email: skipped - {why}")
        elif not to_email:
            summary.email_status = "nothing new since last email"
            print("  email: nothing new to send")
        else:
            sent, detail = mailer_mod.send_digest(to_email, analyses)
            summary.email_status = detail
            if sent:
                # Marked ONLY on success, so a failed send retries next run
                # instead of being silently dropped.
                state.mark_emailed([r.release_key for r in to_email])
                summary.emailed_releases = len(to_email)
                print(f"  email: {detail} covering {len(to_email)} release(s)")
            else:
                print(f"  email: NOT sent - {detail}")
                if "not configured" not in detail:
                    summary.source_errors.append(f"email: {detail}")
    else:
        print(f"  email: [dry-run] {len(state.pending_email())} un-emailed release(s) pending")
