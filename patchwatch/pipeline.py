...
from .deliver import timeline as timeline_mod
...
class RunSummary:
    ...
    def render(self) -> str:
        lines = [
            ...
            f"  timeline entries     +{self.timeline_added}"
            + (f"  -> {self.timeline_path}" if self.timeline_path else ""),
            f"  update              {self.update_status}"
            + (f"  -> {self.update_doc}" if self.update_doc else ""),
...
def run(
    *,
    ...
    min_interval_hours: float = 10.0,
    run_number: str = "",
    history_path: str = "reports/history.json",
    docx_path: str = "reports/PatchTimeline.docx",
    dry_run: bool = False,
    use_nvd: bool = True,
    use_model: bool = True,
    ...
    print(f"patchwatch: platforms={','.join(platforms)} threshold={threshold} "
          f"since_days={since_days} interval={min_interval_hours}h dry_run={dry_run}")
    print("\n[1/5] fetch")
    ...
    for release, status, actionable in work:
        ...
        analyses[release.release_key] = payload
    ...
    pruned = state.prune()
    if pruned["cves_pruned"] or pruned["releases_pruned"]:
        print(f"  pruned {pruned['cves_pruned']} CVE and "
              f"{pruned['releases_pruned']} release records from state")

    # ---- update documents ----------------------------------------------------
    # Gate on interval FIRST. A run inside the window finds releases and records
    # state as usual, but produces no update document - so the cadence the reader
    # sees is exactly min_interval_hours regardless of how often the workflow runs.
    if dry_run:
        print(f"  update: [dry-run] {len(work)} release(s) would be reported")
    else:
        should_run, why = throttle.due(state.last_update_at, min_interval_hours)
        pending = set(state.pending_report())
        to_report = [r for r, _, _ in work if r.release_key in pending]

        if not should_run:
            summary.update_status = f"held ({why})"
            print(f"  update: held - {why}")
        elif not to_report:
            summary.update_status = "no new releases since last update"
            print("  update: nothing new to report")
        else:
            entries = [
                timeline_mod.entry_from_release(r, status, analyses.get(r.release_key, {}))
                for r, status, _ in work if r.release_key in pending
            ]

            # 1. cumulative history + timeline
            history = timeline_mod.load_history(history_path)
            history, added = timeline_mod.append_entries(history, entries)
            summary.timeline_added = added
            timeline_mod.save_history(history_path, history)

            # 2. this update's own document
            # Seconds AND run number in the filename. Minute resolution alone
            # collides when two updates land in the same minute - which happens
            # after a state reset or a manual re-run - and the second silently
            # overwrites the first.
            stamp = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
            suffix = f"-run{run_number}" if run_number else ""
            update_path = (Path(docx_path).parent / "updates"
                           / f"patch-update-{stamp}{suffix}.docx")
            timeline_mod.render_update(entries, update_path, run_number=run_number)

            # 3. mark reported ONLY after both documents exist, so a crash mid-way
            #    re-reports rather than losing the releases entirely.
            state.mark_reported([r.release_key for r in to_report])
            summary.update_status = f"generated ({len(entries)} release(s))"
            summary.update_doc = str(update_path)
            print(f"  update: {len(entries)} release(s) -> {update_path}")

    # Regenerate the cumulative timeline on EVERY run, outside the gate above, so a
    # deleted or corrupted .docx self-heals instead of waiting for the next release.
    if not dry_run:
        history = timeline_mod.load_history(history_path)
        out = timeline_mod.render(history, docx_path)
        summary.timeline_path = str(out)
        print(f"  timeline: {len(history)} total entries -> {out}")
