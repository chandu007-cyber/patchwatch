...
def cmd_preview_email(args: argparse.Namespace) -> int:
    """Render the digest to stdout or an HTML file without sending it.

    Lets you see exactly what recipients would get before touching SMTP config.
    """
    from . import pipeline
    ...
    releases = pipeline.fetch_all(args.platforms.split(","), summary)
    releases = pipeline._apply_window(releases, args.since_days)
    pipeline.enrich_all(releases, summary, use_nvd=False)
    from .severity import resolve_all
    for r in releases:
        resolve_all(r.vulns)
    actionable = [r for r in releases if r.needs_ticket(args.threshold)]

    if not actionable:
        print("No releases would be included in a digest right now.")
        return 0

    print(f"SUBJECT: {mailer.build_subject(actionable, '[patchwatch]')}\n")
    print(mailer._plain_body(actionable, {}))
    if args.html:
        Path(args.html).write_text(mailer._html_body(actionable, {}), encoding="utf-8")
        print(f"\n[HTML version written to {args.html}]")
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_run.add_argument("--history", default="reports/history.json")
    p_run.add_argument("--docx", default="reports/PatchTimeline.docx")
    ...
    p_prev = sub.add_parser("preview-email", help="render the digest without sending")
    p_prev.add_argument("--platforms", default="ios,macos,android,windows")
    p_prev.add_argument("--threshold", default="HIGH", choices=SEVERITY_ORDER)
    p_prev.add_argument("--since-days", type=int, default=30)
    p_prev.add_argument("--html", help="also write the HTML version to this path")
